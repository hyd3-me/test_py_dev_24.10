let currentChart // Глобальная переменная для хранения текущего графика
let currentPopChart // Глобальная переменная для хранения текущего графика

export function createChart(dates, allDatasets) {
  const iconActive = "/static/img/svg/glass2.svg";
  const iconInActive = "/static/img/svg/glass.svg";
        ...
        tooltip: {
          ...
          callbacks: {
            label: function (context) {
              return `${context.dataset.label}: ${context.formattedValue}`; // Формат текста
            },
          },
        },
      },
  ...
  const visibleDataPoints = 12; // Видимая область (например, 10 элементов)
  ...
  if (currentChart || currentPopChart) {
    currentChart.destroy()
    currentPopChart.destroy()
  }
  currentChart = new Chart(ctx, cfg);
  currentPopChart = new Chart(ctx_pop, cfg_pop);
  ...
  function showDataset(datasetIndices) {
    ...
    currentPopChart.data.datasets = newDatasets;
    currentPopChart.update(); // Обновляем график после изменения видимости
  }
  ...
  function moveProgressBar(e) {
    ...
    currentPopChart.options.scales.x.min = newMin;
    currentPopChart.options.scales.x.max = newMax;
    currentPopChart.update();
  
    // Обновляем положение прогресс-бара
    progressBar.style.width = `${progressPercentage * 100}%`;
  }
  
  let positionsGraphic = () => {
