document.addEventListener("DOMContentLoaded", () => {
  ...
  // События на родительском элементе динамических позиций
  const container = document.querySelector('.positions')
  let itemPosActiveCount = 0;
  if (container) {
    container.addEventListener('click', (event) => {
      if (event.target.closest('.copy-item')) {
        copyText(event.target)
      }
      if (event.target.closest('.checkbox')) {
        //Чекбоксы отдельных позиций
  
        if (event.target.closest(".all-positions")) {
          choseAllPositions();
        }
        if (event.target.closest(".item-pos")) {
          itemsEvent(event.target);
        }
        if (event.target.closest(".edit .edit__positions")) {
          editPosChose();
          editAllPosChose();
        }
      }
  ...
  const form = document.getElementById("project-form-auto-f"); // Создание конфига вручную
  const form2 = document.getElementById("project-form-auto-t"); // Создание конфига автоматически
  const form3 = document.getElementById("project-form-new-project"); // Выбор названия и проекта
  const wProjectSubmit = document.querySelector('#createWproject')

  async function grouped_configs() {
    const selectedFolder = document.querySelector("#wFolders");
    const foldersDropdown = selectedFolder.querySelector('.project__dropdown')
    fetch(foldersDropdown.dataset.urlgetconfigs)
    .then(response => response.json())
    .then(data => {
        let container = foldersDropdown.querySelector('.search__folders-wrap')
        for (let title of Object.keys(data)) {
            container.appendChild(setFolder(title))
          }
        foldersDropdown.insertBefore(container, foldersDropdown.querySelector('.btn-new-folder'))
  ...
  form.addEventListener("submit", function (event) {
    ...
    //   }
    // }
    userInput['access_token'] = document.querySelector('[name=Accesstoken]').value
    userInput['user_id'] = document.querySelector('[name=UserID]').value
    userInput['host_id'] = document.querySelector('[name=HostID]').value
  ...
  form3.addEventListener("submit", function (event) {
    ...
    const input1 = document.querySelector(".name-project-input");
    const input2 = document.querySelector("#wFolders");
    ...
    if (input1.value <= 0) {
      ...
      if (
        input2.querySelector("span").textContent == "Выберите или создайте папку для проекта"
      ) {
        ...
        document.querySelector(".input-title__error-6").classList.remove("active");
    ...
    function submitForm(userData) {
        const nameInfo = {
            name: input1.value,
            database_name: `${input1.value}_db`,
            new_group_name: input2.querySelector('.select-project-btn').outerText,
        };

        fetch(wProjectSubmit.dataset.url, {
            method: "POST",
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({...nameInfo, ...userData})
        }).then(response => {
            if (!response.ok) {
        ...
        switch (true) {
          case thisQuizElement.classList.contains("pop-pos__quiz-1"):
            ...
            if (input1.value.length <= 0) {
              ...
              posQuizData.listName = input1.value
            }
            if (input2.value.length <= 0) {
              ...
              posQuizData.mainDomain = input2.value
            }
          ...
          case thisQuizElement.classList.contains("pop-pos__quiz-2"):
            ...
            if (error === true) return;
            posQuizData.query_list = input3.value.split('\n').filter(uri => uri.trim() !== '')
            break;
          ...
          case thisQuizElement.classList.contains("pop-pos__quiz-4"):
            error = false;
            const regions = thisQuizElement.querySelectorAll(".region");
            ...
            if (!regions) {
              ...
              regions.forEach(item => {
                let region_id = item.querySelector('.regions-btn>span').id
                let region_browser = item.querySelector('.region__browser').dataset.search_system
                posQuizData.regions[region_browser].push(region_id)
              })
              console.log(posQuizData)
              document
               .querySelector(".regions")
          ...
          case thisQuizElement.classList.contains("pop-pos__quiz-5"):
            ...
            if (
              input5.textContent.replace(/\s+/g, "") == "Потребованию"
            ) {
              error = false;
            }
            if (error === true) return;
            createLiveSearch(posQuizData)
            break;
        }
  ...
  function createLiveSearch(data) {
    const url = document.querySelector(".pop-pos").dataset.url
    fetch(url, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
  ...
  // let regionPopup = document.querySelector('#regionsPopup') 
  // let regionItems = fetch(regionPopup.dataset.getregionsurl).then(response => response.json().then(data => data.data));
  ...
  const createRegion = (button, body) => {
    ...
    addButton.addEventListener("click", async () => {
      ...
      if (button == ".pop-pos .regions__add") {
        region.innerHTML = `
          <div data-search_system="${!browserGoogle ? 'Yandex' : 'Google'}" class="region__browser">
          ${browserInner}
          </div>
      ...
      `;
        ...
        region.innerHTML = `
            ...
            <div class="search-folders regions-menu">
              <div class="select-btn regions-btn btn-reset" style="border-color: rgb(0, 118, 245);">
                <span data-search_system="Google">Выберите регион</span>
                <svg class="project__btn-icon" width="16" height="16">
                  <use href="/static/img/svg/sprite.svg#arrow_dropdown"></use>
                </svg>
              </div>
        ...
        region.innerHTML = `
            ...
            <div class="search-folders regions-menu">
              <div class="select-btn regions-btn btn-reset" style="border-color: rgb(0, 118, 245);">
                <span data-search_system="Yandex">Выберите регион</span>
                <svg class="project__btn-icon" width="16" height="16">
                  <use href="/static/img/svg/sprite.svg#arrow_dropdown"></use>
                </svg>
              </div>
