...
async def get_queries_test(
    ...
    DATABASE_NAME = request.session['config'].get('database_name', "")
    config_id = request.session['config'].get('config_id', '')

    config_names = [elem for elem in (
        await get_config_names(session, user, group_name)
    )]
    ...
    return templates.TemplateResponse(
        "/templates/get_queries_test/query.html", {
            "request": request,
            "user": user,
            "config_names": config_names,
            "group_names": group_names,
...
@router.post("/get_total_sum_test/")
async def get_total_sum_test(
    request: Request, 
    data_request: dict, 
    user: User = Depends(current_user),
    general_session: AsyncSession = Depends(get_db_general),
    ...
    if data_request["search_text"] == "":
        ...
        not_void_count_metricks = await _get_not_void_count_daily_summary_like(
                    start_date, end_date,
                    data_request["search_text"], async_session,
                )
    ...
    def create_dict(title):
        return {"query": title}

    # Создание словарей для каждого заголовка
    res_clicks = create_dict("Суммарные клики")
    res_impressions = create_dict("Суммарные показы")
    res_not_void = create_dict("Строки в топ 50")
    res_top_3 = create_dict("Строки в топ 3")
    res_top_5 = create_dict("Строки в топ 5")
    res_top_10 = create_dict("Строки в топ 10")
    ...
    for (
        ...
        clicks_count,
        impressions_count
    ) in sorted(metricks, key=lambda x: x[0]):
        clicks[date.strftime(date_format_2)] = clicks_count
        impressions[date.strftime(date_format_2)] = impressions_count

        if clicks_count > 0:
            res_clicks[date.strftime(date_format_2)] = clicks_count
            total_clicks_days += clicks_count
        else:
            res_clicks[date.strftime(date_format_2)] = 0

        if impressions_count > 0:
            res_impressions[date.strftime(date_format_2)] = impressions_count
            total_impession_days += impressions_count
        else:
            res_impressions[date.strftime(date_format_2)] = 0
    ...
    for (
        ...
        top_5_count,
        top_10_count
    ) in sorted(not_void_count_metricks, key=lambda x: x[0]):
        date_str = date.strftime(date_format_2)

        # Обработка для общего not_void
        not_void[date_str] = total_count
        total_not_void += total_count
        res_not_void[date_str] = total_count

        # Обработка для top-3
        top_3[date_str] = top_3_count
        res_top_3[date_str] = top_3_count

        # Обработка для top-5
        top_5[date_str] = top_5_count
        res_top_5[date_str] = top_5_count

        # Обработка для top-10
        top_10[date_str] = top_10_count
        res_top_10[date_str] = top_10_count

    # Итоговые значения для кликов, показов и всех топов
    res_clicks["result"] = total_clicks_days
    res_impressions["result"] = total_impession_days
    res_not_void["result"] = total_not_void
    res_top_3["result"] = sum(top_3.values())
    res_top_5["result"] = sum(top_5.values())
    res_top_10["result"] = sum(top_10.values())
    ...
    metricks_data = [
        res_clicks, res_impressions,
        res_not_void, res_top_10,
        res_top_5, res_top_3
    ]
...
async def get_queries_test(
    ...
    try:
        if not urls:
            return HTTPException(
                status_code=400,
                detail="Запрос не вернул результатов"
            )
    ...
    for el in grouped_data:
        ...
        for k, stat in enumerate(el[1]):
            ...
            if stat[1] <= 3:
                color_text = "blue"
            if not all(map(lambda x: float(x) == 0.0, stat[1:-1])):
            ...
            if k == len(el[1]) - 1:
                res["result"] = {
                    'id': k,
                    'title': el[0],
                    'total_position': round(position / count, 2) if count else 0,
                    'total_clicks': total_clicks,
                    'total_impressions': impressions,
                    'total_CTR': round(total_clicks * 100 / impressions, 2) if impressions else 0,
                }
        data.append(res)
...
@router.get('/edit_query/{config_id}')
async def edit_query(
    ...
    config_query = select(Config).where(Config.id == config_id)
    config_data = (await session.execute(config_query)).scalars().first()

    if not config_data:
        raise HTTPException(
            status_code=404,
            detail='Конфиг не был найден'
        )

    if user.id != config_data.id_author:
        raise HTTPException(
            status_code=403,
            detail='Конфиг был создан не Вами'
        )

    selected_group_query = (
        select(Group.name)
        .where(Group.id == (
            select(GroupConfigAssociation.group_id)
            .where(GroupConfigAssociation.config_id == config_id)
        ))
    )
    selected_group_name = (
        await session.execute(selected_group_query)
    ).scalars().first()

    groups_data = (await session.execute(select(Group))).scalars().all()

    return templates.TemplateResponse(
        '/templates/get_queries_test/edit.html', {
            'request': request,
            'user': user,
...
@router.post('/edit_query/{config_id}')
async def save_edit_query(
    request: Request,
    config_id: int,
    config_data: dict,
    user: User = Depends(current_user),
    ...
    try:
        group_name = {'name': config_data.pop('group_name')}
        config_record = (await session.execute(
            select(Config)
        ...
        group_record = (await session.execute(
            select(Group)
            .where(Group.name == group_name["name"])
        )).scalar_one_or_none()
        if group_record:
            await session.execute(
                update(GroupConfigAssociation)
                .where(GroupConfigAssociation.config_id == config_id)
                .values(group_id=group_record.id)
            )
        else:
            new_group = Group(id_author=user.id, **group_name)
